        ...
        if (sock->state == LISTEN) {
            if (sock->buffer_size > 0) {
                ...
                if (pdu_syn.type == SYN) {
                    ...
                    mic_tcp_pdu_t pdu_synack;
                    pdu_synack.type = SYN_ACK;
                    pdu_synack.seq_num = sock->next_seq_num;
                    pdu_synack.ack_num = pdu_syn.seq_num + 1;
                    ...
                    // Attente de l'ACK
                    mic_tcp_pdu_t pdu_ack;
                    mic_tcp_sock_addr addr_ack;
                    IP_recv(&pdu_ack, &addr_ack, timeout)
                   
                    
                    // Vérification du type de PDU
                    if (pdu_ack.type == ACK && pdu_ack.ack_num == sock->next_seq_num) {
